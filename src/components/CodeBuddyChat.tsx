...
export function CodeBuddyChat() {
  ...
  const { openedTabs, activeTabId, getTabContent, refreshTabContent } = useEditor(); // Ensure refreshTabContent is available in EditorContext
  ...
  const extractCleanCode = (code: string): string => {
    ...
    const filePathLineIndex = lines.findIndex(line => line.trim().startsWith('FILE_PATH:'));
    const operationLineIndex = lines.findIndex(line => line.trim().startsWith('OPERATION:'));

    if (filePathLineIndex !== -1 && operationLineIndex !== -1) {
        contentStartIndex = Math.max(filePathLineIndex, operationLineIndex) + 1;
        // Find the first empty line after metadata lines to start content from there
  ...
  const handleAcceptCode = (code: string, index: number) => {
    ...
    try {
      ...
      if (existingFileId) {
        ...
        // Refresh the tab content for the updated file
        refreshTabContent(existingFileId); // Call refreshTabContent with the file ID

        toast.success('File updated successfully', {
          description: `Updated ${filePath}`,
          icon: <CheckCircle size={18} />
        });
      } else if (operation.toUpperCase() === 'CREATE') { // Only create if operation is CREATE
        // Create new file
        ...
        if (newFileId) {
          ...
          selectFile(newFileId); // Automatically select and open the new file
        ...
        toast.error('Cannot create file', {
          description: `File path detected but operation was not CREATE. Ensure operation is specified as CREATE for new files.`
        });
      }
