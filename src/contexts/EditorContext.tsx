...
export const EditorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateTabContent = (tabId: string, content: string) => {
    ...
    setOpenedTabs(prevTabs => 
      prevTabs.map(tab => 
        tab.id === tabId 
          ? { ...tab, content: processedContent, isModified: true } 
          : tab
      )
    );
    
    sessionStorage.setItem(`${STORAGE_KEY_PREFIX}${tabId}`, processedContent);
    
    updateFileContent(tabId, processedContent);
    
    if (monacoInstance && activeTabId === tabId) {
      monacoInstance.setValue(processedContent);
    }
    
    const event = new CustomEvent('content-updated', {
      detail: { tabId, content: processedContent }
    });
